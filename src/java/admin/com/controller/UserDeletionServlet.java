...
public class UserDeletionServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (rows > 0) {
                response.sendRedirect("UserDeletion.jsp?message=Record+deleted+successfully");
            } else {
                response.sendRedirect("UserDeletion.jsp?message=No+record+found+with+that+ID");
            }

        } catch (Exception e) {
            //e.printStackTrace();
            response.getWriter().println("SQL Error: " + e.getMessage());
            response.sendRedirect("UserDeletion.jsp?message=Error+occurred+while+deleting");
        }
    }
